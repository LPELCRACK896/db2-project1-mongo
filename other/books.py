...
def book_post(filename_authors, total_books):
    ...
    for _ in range(total_books):
        ...
        book = {"title": faker.sentence(nb_words=4), "author": author_id, "puslisher": faker.word(), "pages": faker.random_int(min=18, max=65, step=1),"year": faker.random_int(min=1800, max=2023, step=1), "isbn": faker.isbn10(separator=''), "desc": ' '.join([faker.sentence() for _ in range(random.randint(4, 6))]), "category": random.choice(categorias)}
